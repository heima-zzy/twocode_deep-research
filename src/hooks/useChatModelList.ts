...
function useChatModelList() {
  const [modelList, setModelList] = useState<string[]>([]);
  const { mode, provider } = useChatSettingStore.getState();
  ...
  async function refresh(provider: string): Promise<string[]> {
    const { accessPassword } = useChatSettingStore.getState();
    const accessKey = generateSignature(accessPassword, Date.now());

    if (provider === "google") {
      const { googleApiKey = "", googleApiProxy } = useChatSettingStore.getState();
      if (mode === "local" && !googleApiKey) {
        return [];
      }
      const key = multiApiKeyPolling(googleApiKey);
      const response = await fetch(
        mode === "local"
          ? completePath(googleApiProxy || GEMINI_BASE_URL, "/v1beta") + "/models"
      ...
      const { openAIApiKey = "", openAIApiProxy } = useChatSettingStore.getState();
      ...
      const { xAIApiKey = "", xAIApiProxy } = useChatSettingStore.getState();
      ...
      const { pollinationsApiProxy } = useChatSettingStore.getState();
      ...
      const { ollamaApiProxy } = useChatSettingStore.getState();
      const headers = new Headers();
      if (mode === "proxy") headers.set("Authorization", `Bearer ${accessKey}`);
      const response = await fetch(
        mode === "proxy"
...
export { useChatModelList };
export default useChatModelList;